...
public class PolicyServiceCore implements PolicyService {
    ...
    public PolicyServiceCore(DataManager dataManager, Sequences sequences, ApplicationEventPublisher eventPublisher) {
    ...
    public PolicyDto createPolicy(CreatePolicyRequestDto request) {
        ...
        if (insuranceProduct == null) {
            throw new IllegalArgumentException("Unknown insurance product: " + request.insuranceProductId());
        }

        PaymentFrequency paymentFrequency = PaymentFrequency.fromId(request.paymentFrequencyId());
        if (paymentFrequency == null) {
            throw new IllegalArgumentException("Unknown payment frequency: " + request.paymentFrequencyId());
        }
        ...
        String policyNo = "%s-%s-%06d".formatted(productCode, request.effectiveDate().getYear(), policySequenceNumber);
