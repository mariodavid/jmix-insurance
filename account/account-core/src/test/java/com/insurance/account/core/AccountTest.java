...
class AccountTest {
  ...
  private static final java.util.UUID POLICY_ID_QUARTERLY =
      java.util.UUID.fromString("00000000-0000-0000-0000-000000000003");
  private static final java.util.UUID POLICY_ID_BALANCE =
  ...
  void given_quarterlyFrequency_when_accountCreated_then_fourDocumentsSpreadOverYear() {
    ...
    assertThat(account).hasBalance(PREMIUM.negate()).hasDocumentCount(4);
  ...
  void given_monthlyFrequency_when_accountCreated_then_twelveDocumentsSpreadOverYear() {
    ...
    assertThat(account).hasBalance(PREMIUM.negate()).hasDocumentCount(12);
  ...
  void given_quarterlyAccount_when_balanceQueriedBeforeThirdPayment_then_partialSumReturned() {
    ...
    // when
    BigDecimal balance =
        accountService.getAccountBalance("HC-2025-000004", LocalDate.of(2025, 6, 30));
