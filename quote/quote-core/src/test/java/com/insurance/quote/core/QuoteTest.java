...
class QuoteTest {
  ...
  @Test
  void
      given_pendingQuoteAndPolicyServiceResponse_when_accepted_then_quoteHasAcceptedStatusAndPolicyReference() {
    Quote quote =
        entityTestData.saveWithDefaults(new QuoteDataProvider(), q -> q.setPartnerNo("PT-40001"));
    UUID policyId = UUID.randomUUID();
    PolicyDto policyDto = dataManager.create(PolicyDto.class);
    policyDto.setId(policyId);
    policyDto.setPolicyNo("HC-2025-000123");
    when(policyService.createPolicy(org.mockito.ArgumentMatchers.any())).thenReturn(policyDto);
    ...
    assertThat(requestCaptor.getValue().insuranceProductId())
        .isEqualTo("HOME_CONTENT_BASIC_2024_01");
  ...
  @Test
  void
      given_pendingQuoteAndPolicyServiceThrowsException_when_accepted_then_exceptionIsThrownAndStatusRemainsPending() {
    Quote quote = entityTestData.saveWithDefaults(new QuoteDataProvider());
