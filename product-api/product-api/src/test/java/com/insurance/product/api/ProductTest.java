...
class ProductTest {
    ...
    @Test
    void given_productTypeVariantAndEffectiveDate_when_findFirstMatchingProduct_then_matchingProductIsReturnedOnlyWithinValidity() {
        Optional<InsuranceProduct> valid = InsuranceProduct.findFirstMatchingProduct(
                ProductType.HOME_CONTENT,
                ProductVariant.SMALL,
                LocalDate.of(2024, 1, 1));
        Optional<InsuranceProduct> invalidDate = InsuranceProduct.findFirstMatchingProduct(
                ProductType.HOME_CONTENT,
    ...
    Stream<DynamicTest> productModulesDoNotContainPartnerImportsOrPartnerViewResources() throws IOException {
        ...
        return files.stream()
                .map(path -> DynamicTest.dynamicTest(repositoryRoot.relativize(path).toString(), () -> {
