...
public class PartnerServiceCore implements PartnerService {
  ...
  public PartnerServiceCore(
      PartnerRepository partnerRepository, DataManager dataManager, Sequences sequences) {
  ...
  public List<PartnerDto> findPartners(String search, int limit, int offset) {
    ...
    List<Partner> partners =
        loader.firstResult(offset >= 0 ? offset : 0).maxResults(limit > 0 ? limit : 20).list();
  ...
  public PartnerDto getPartner(String partnerNo) {
    return partnerRepository.findByPartnerNo(partnerNo).map(this::mapToDto).orElse(null);
  }
  ...
  public PartnerDto savePartner(PartnerDto partnerDto) {
    ...
    if (partnerDto.getId() != null) {
      partner = dataManager.load(Partner.class).id(partnerDto.getId()).optional().orElse(null);
    }
