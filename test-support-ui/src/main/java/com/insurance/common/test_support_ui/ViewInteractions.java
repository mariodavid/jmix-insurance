...
public class ViewInteractions {
  ...
  public <S extends StandardDetailView<E>, E> S openDetailForCreation(
      Class<S> viewClass, Class<E> entityClass) {
    DialogWindow<S> dialogWindow =
        dialogWindows.detail(parentView, entityClass).withViewClass(viewClass).newEntity().build();
    return dialogWindow.getView();
  }

  public <S extends StandardDetailView<E>, E> S openDetailForEditing(
      Class<S> viewClass, Class<E> entityClass, E entity) {
    DialogWindow<S> dialogWindow =
        dialogWindows
            .detail(parentView, entityClass)
